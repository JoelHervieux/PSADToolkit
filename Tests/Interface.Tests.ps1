# Exercise the actual GUI field definitions without launching GliderUI or Avalonia.
BeforeAll {
    ...
    if ($errors) { throw ('Start-PSADToolkit.ps1 : ' + ($errors.Message -join '; ')) }
...
Describe 'GUI field definitions' {
    ...
    It 'Only uses control kinds the GliderUI builder knows how to create' {
        $known = @('text', 'list', 'check', 'number', 'delimiter', 'ou', 'open', 'savecsv', 'savehtml', 'folder')
        foreach ($spec in $specs) {
            foreach ($field in $spec.Fields) {
                $known -contains $field[2] | Should -BeTrue -Because ('{0} / {1}' -f $spec.Title, $field[0])
            }
        }
    }
    It 'Targets only functions the module exports' {
        foreach ($spec in $specs) {
            $manifest.FunctionsToExport -contains $spec.Command | Should -BeTrue -Because $spec.Title
        }
    }
}
